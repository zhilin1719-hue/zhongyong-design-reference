...
const safeURL = raw => { try {const u = new URL(raw, location.origin); return ['http:', 'https:'].includes(u.protocol) ? u.href : null;} catch {return null;} };
...
function bubble(text,role='assistant',sources=[]){const node=document.createElement('div');node.className='chat-bubble '+(role==='user'?'user':'assistant');node.textContent=text;if(Array.isArray(sources)&&sources.length){const links=document.createElement('div');links.className='chat-sources';sources.forEach(source=>{const href=safeURL(source.url);if(!href)return;const a=document.createElement('a');a.textContent=source.title||'查看来源';a.href=href;a.target='_blank';a.rel='noopener noreferrer';links.append(a);});node.append(links);}messages.append(node);messages.scrollTop=messages.scrollHeight;return node;}
function handoff(raw){const a=$('#handoff-link'),url=raw&&safeURL(raw);a.hidden=!url;if(url)a.href=url;}
async function loadHistory(){if(loading||loaded)return;loading=true;$('#chat-form button').disabled=true;note('正在载入咨询记录…');try{const data=await request('/api/chat/');messages.replaceChildren();if(Array.isArray(data.messages)&&data.messages.length){data.messages.filter(m=>['user','assistant'].includes(m.role)&&typeof m.content==='string').forEach(m=>bubble(m.content,m.role,m.sources));}else{bubble('您好，欢迎来到中用设计。您可以咨询我们的服务、项目作品或合作方式。');}handoff(data.handoff_url);loaded=true;note('');}catch(e){note(e.message,loadHistory);}finally{loading=false;$('#chat-form button').disabled=false;}}
function openChat(){activeBefore=document.activeElement;panel.hidden=false;backdrop.hidden=false;modalBackground.forEach(node=>node.inert=true);launch.setAttribute('aria-expanded','true');document.body.classList.add('modal-open');input.focus();loadHistory();}
...
let sending=false;
async function sendMessage(text,append=true){if(sending)return;sending=true;const btn=$('#chat-form button');btn.disabled=true;input.disabled=true;if(append)bubble(text,'user');note('正在查询公开资料…');try{const data=await request('/api/chat/',{message:text});if(typeof data.reply!=='string')throw new Error('未收到有效答复，请重试。');bubble(data.reply,'assistant',data.sources);handoff(data.handoff_url);input.value='';note('');}catch(e){note(e.message,()=>sendMessage(text,false));input.value=text;}finally{sending=false;btn.disabled=false;input.disabled=false;if(!panel.hidden)input.focus();}}
